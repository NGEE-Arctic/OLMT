...
parser.add_option(
    "--n_ensemble", dest="n", default=0, help="Number of ensemble members"
)
parser.add_option("--case", dest="casename", default="", help="Name of case")
parser.add_option(
    "--ens_file", dest="ens_file", default="", help="Name of samples file"
)
parser.add_option(
    "--mc_ensemble", dest="mc_ensemble", default=0, help="Create monte carlo ensemble"
)
parser.add_option(
    "--microbe",
    dest="microbe",
...
parser.add_option("--site", dest="site", default="", help="Site name")
parser.add_option(
    "--spruce_treatments",
    dest="spruce_treatments",
    default=False,
    action="store_true",
    help="Run 11 SPRUCE treatment simulations",
)
parser.add_option(
    "--run_uq",
    dest="run_uq",
...
def postproc(
    ...
    baserundir = (
        options.runroot + "/UQ/" + case + "/g" + str(100000 + thisjob)[1:] + "/"
    )
    index = 0
    ierr = 0
    thiscol = 0
            ...
            if npy == 365:
                for d in range(myday_start[index] - 1, myday_end[index]):
                    if "US-SPR" in case and "ZWT" in v:
                        ...
                        output.append(
                            mydata[d][myindex + hol_add] * myfactor[index]
                            + myoffset[index]
                            + mydata2[d][myindex + hol_add] / 1000.0
                        )
                    elif "US-SPR" in case:
                        output.append(
                            0.25
                            * (
                            ...
                            + 0.75
                            * (mydata[d][myindex] * myfactor[index] + myoffset[index])
                        )
                    else:
                        output.append(
                            mydata[d][myindex] * myfactor[index] + myoffset[index]
                ...
                for d in range(
                    myday_start[index] - 1, myday_end[index]
                ):  # 28-38 was myindex
                    if "SCPF" in v:
                        output.append(
                            sum(mydata[0, 28:38]) / 10.0 * myfactor[index]
                            + myoffset[index]
                        )
                    elif "NPLANT_SCLS" in v:
                        output.append(
                            sum(mydata[0, 1:]) * myfactor[index] + myoffset[index]
                        )
                    elif "SCLS" in v:
                        output.append(
                            sum(mydata[0, :]) * myfactor[index] + myoffset[index]
                        )
                    else:
                        try:
                            output.append(
            ...
            if p == "lai":  # Surface data file
                ...
                if int(ppfts[pnum]) >= 0:
                    if "fates_prt_nitr_stoich_p1" in p:
                        ...
                        parms[pnum] = mydata[
                            int(ppfts[pnum]) / 12, int(ppfts[pnum]) % 12
                        ]
                    elif (
                        "fates_hydr_p50_node" in p
                        or "fates_hydr_avuln_node" in p
                        or "fates_hydr_kmax_node" in p
                        or "fates_hydr_pitlp_node" in p
                        or "fates_hydr_thetas_node" in p
                    ):
                        parms[pnum] = mydata[
                            int(ppfts[pnum]) / 12, int(ppfts[pnum]) % 12
                        ]
...
if os.path.isfile(options.postproc_file):
    ...
    for s in postproc_input:
        if s[0:1] != "#":
            ...
            days_total = (int(s.split()[2]) - int(s.split()[1]) + 1) * (
                int(s.split()[4]) - int(s.split()[3]) + 1
            )
...
if rank == 0:
    ...
    if do_postproc:
        ...
        np.savetxt(
            UQ_output + "/data/ytrain.dat", data_out[0 : int(len(good) * 0.8), :]
        )
        np.savetxt(UQ_output + "/data/yval.dat", data_out[int(len(good) * 0.8) :, :])
        np.savetxt(
            UQ_output + "/data/ptrain.dat", parm_out[0 : int(len(good) * 0.8), :]
        )
        ...
        np.savetxt(
            UQ_output + "/data/foreden.csv",
            np.hstack((parm_out, data_out)),
            delimiter=",",
            header=eden_header[:-1],
        )
        if options.run_uq:
                                ...
                                for s in lnd_in_old:
                                    if "finidat =" in s:
                                        ...
                                        if "CO2" in treatments[t]:
                                            ...
                                            lnd_in_new.write(
                                                " startdate_add_co2 = '20160315'\n"
                                            )
                                    elif "landuse_timeseries" in s:
                                        lnd_in_new.write(
                                            s.replace("plot07", "plot" + pst)
                                        )
                                    else:
                                        lnd_in_new.write(s)
                                lnd_in_old.close()
