...
parser.add_option(
    ...
    dest="nopointdata",
    default=False,
    help="do not generate point data",
)
parser.add_option(
    "--makepointdata_only",
    action="store_true",
...
parser.add_option("--region", dest="region", default="", help="Set pre-defined region")
parser.add_option(
    "--runblock",
    dest="runblock",
    default=9999,
...
parser.add_option(
    "--parm_file_P", dest="parm_file_P", default="", help="P parameter file to use"
)
parser.add_option(
...
parser.add_option(
    "--res", dest="res", default="hcru_hcru", help="Resoultion for global simulation"
)
parser.add_option("--np", dest="np", default=256, help="number of processors")
...
parser.add_option(
    "--BGC", action="store_true", dest="bgc", default=False, help="Use BGC compset"
)
parser.add_option(
    "--SP",
    dest="sp",
...
parser.add_option(
    "--ECA", action="store_true", dest="eca", default=False, help="Use ECA compset"
)
parser.add_option(
    "--centbgc",
    dest="centbgc",
...
if translen == -1:
    ...
    if options.cpl_bypass and (
        options.cruncep or options.cruncepv8 or options.gswp3 or options.princeton
    ):
...
basecmd = (
    "python runcase.py --surfdata_grid --ccsm_input "
    + os.path.abspath(ccsm_input)
    + " --rmold --no_submit --machine "
    + options.machine
    + " --res "
    + options.res
    + " --model_root "
...
if options.noad:
    if options.sp:
        ...
        cmd_fnsp = (
            ...
            + " --align_year "
            + str(year_align + 1)
            + " --coldstart --run_startyear "
            + str(options.run_startyear)
        )
    else:
        cmd_fnsp = (
            basecmd
    ...
    if options.exeroot != "":
        cmd_fnsp = (
            cmd_fnsp + " --no_build --exeroot " + os.path.abspath(options.exeroot)
        )
        ad_exeroot = os.path.abspath(options.exeroot)
    else:
...
if options.finidat != "":  # transient simulation only, must provide restart
    ...
    if options.exeroot != "":
        cmd_trns = (
            cmd_trns + " --no_build --exeroot " + os.path.abspath(options.exeroot)
        )
        ad_exeroot = os.path.abspath(options.exeroot)
    else:
        ad_exeroot = os.path.abspath(runroot + "/" + basecase)
...
if (
    (options.cruncep or options.cruncepv8)
    and not options.cpl_bypass
    and not options.notrans
):
    ...
    for c in cases:
        ...
        for n in range(0, n_submits):
            ...
            for s in input:
                if "perl" in s or "python" in s:
                    timestr = (
                        ...
                        + str(
                            int(
                                (float(options.walltime) - int(float(options.walltime)))
                                * 60
                            )
                        )
                        + ":00"
            ...
            if options.cplhist:
                ...
                output.write(
                    "./xmlchange --id REST_N --val 20\n"
                )  # Restart every 20 years in global mode
            output.write(
                "./xmlchange --id RUN_STARTDATE --val "
                + (str(10000 + model_startdate))[1:]
                + "-01-01\n"
            )
            if n > 0:
                # change finidat
            ...
            if not options.no_submit:
                os.system(
                    "chmod u+x " + tempdir + "/global_" + c + "_" + str(n) + ".pbs"
                )
                job_depend_run = submit(
                    tempdir + "/global_" + c + "_" + str(n) + ".pbs",
