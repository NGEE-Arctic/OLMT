...
parser.add_option("--ens_num", dest="ens_num", default=1, help="Ensemble member number")
...
parser.add_option("--site", dest="site", default="parm_list", help="Site name")
parser.add_option(
    "--model_name",
    dest="model_name",
    default="clm2",
...
for f in os.listdir(ens_dir):
    if os.path.isfile(ens_dir + "/" + f) and (
        ...
        for s in myinput:
            if "fates_paramfile" in s:
                ...
                if finidat_file_orig.strip() != "":
                    finidat_file_new = (
                        ens_dir + "/" + (finidat_file_orig.split("/")[-1:])[0]
                    )
                    ...
                    if (
                        ("1850" in casename or "CROP" in casename)
                        and "ad_spinup" not in casename
                        and not ("trans" in casename or "20TR" in casename)
                    ):
                        finidat_file_path = (
                            os.path.abspath(options.runroot)
                            + "/UQ/"
                            + casename.replace("1850CNP", "1850CN")
                            + "_ad_spinup/g"
                            + gst[1:]
                        )
...
for p in parm_names:
    if "INI" in p:
        ...
        if ("fates_prt" in p and "stoich" in p) or (
            "fates_turnover" in p and "retrans" in p
        ):
