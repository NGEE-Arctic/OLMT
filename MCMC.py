...
parser.add_option(
    "--nevals", dest="nevals", default="200000", help="Number of model evaluations"
)
parser.add_option(
    "--burnsteps", dest="burnsteps", default="10", help="Number burn steps"
)
parser.add_option(
    "--parm_list",
    dest="parm_list",
    ...
    if prior > 0.0:
        ...
        for v in range(0, len(myoutput)):
            if abs(myobs[v]) < 1e5 and myerr[v] > 0:
                ...
                ri = (resid / myerr[v]) ** 2
                li = -1.0 * np.log(2.0 * np.pi) / 2.0 - np.log(myerr[v]) - ri / 2.0
                post = post + li
    else:
        post = -9999999
    return post
...
def MCMC(parms, nevals, type="uniform", nburn=1000, burnsteps=10, default_output=[]):
    ...
    for i in range(0, nevals):
        ...
        if i == burnsteps * nburn:
            ...
            for p in range(0, nparms):
                ...
                plt.savefig(
                    UQ_output
                    + "/MCMC_output/plots/chains/burnin_chain_"
                    + model.parm_names[p]
                    + ".pdf"
                )
                plt.close(fig)
    ...
    parm_best.close()
    np.savetxt(
        UQ_output + "/MCMC_output/correlation_matrix.txt", np.corrcoef(chain_afterburn)
    )
    ...
    for p in range(0, nparms):
        ...
        plt.savefig(
            UQ_output
            + "/MCMC_output/plots/chains/chain_"
            + model.parm_names[p]
            + ".pdf"
        )
        plt.close(fig)
    ...
    for p in range(0, model.nobs):
        out95.write(
            str(output_sorted[p, int(0.025 * (nevals - nburn * burnsteps))])
            + " "
    ...
    for p in range(0, nparms):
        ...
        plt.savefig(
            UQ_output + "/MCMC_output/plots/pdfs/" + model.parm_names[p] + ".pdf"
        )
        plt.close(fig)
    ...
    for s in range(0, len(obs_set)):
        ...
        ax.errorbar(
            x, model.obs[thisob], yerr=model.obs_err[thisob], label="Observations"
        )
        ax.plot(x, output_best[thisob], "r", label="Model best")
        ax.plot(
            x,
            output_sorted[thisob, int(0.025 * (nevals - nburn * burnsteps))],
            "k--",
            label="Model 95% CI",
        )
        ax.plot(
            x, output_sorted[thisob, int(0.975 * (nevals - nburn * burnsteps))], "k--"
        )
        ...
        plt.savefig(
            UQ_output
            + "/MCMC_output/plots/predictions/Predictions_"
            + obs_set[s]
            + ".pdf"
        )
        plt.close(fig)
...
if options.parm_default != "":
    ...
    parms = MCMC(
        model.pdef,
        int(options.nevals),
        burnsteps=int(options.burnsteps),
