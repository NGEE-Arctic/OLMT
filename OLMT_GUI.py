...
class Frame(wx.Frame):
    ...
    def __init__(self, title):
        ...
        self.m_spinup = wx.Choice(
            panel,
            -1,
            choices=["ad spinup", "final spinup", "transient", "full simulation"],
        )
        ...
        m_text10 = wx.StaticText(
            panel, -1, "Site-specific options", style=wx.ALIGN_LEFT
        )
        ...
        self.sitedata = wx.CheckBox(
            panel, -1, "Use site-level data in surface data files"
        )
        ...
        # pft physiology file entry
        m_text8 = wx.StaticText(
            panel, -1, "File for parameter mods", style=wx.ALIGN_LEFT
        )
        m_text8.SetFont(wx.Font(10, wx.SWISS, wx.NORMAL, wx.BOLD))
        m_text8.SetSize(m_text4.GetBestSize())
        ...
        m_text30 = wx.StaticText(
            panel, -1, "Uncertainty Quantification", style=wx.ALIGN_LEFT
        )
        ...
        m_text32 = wx.StaticText(
            panel, -1, "Number of ensemble members", style=wx.ALIGN_LEFT
        )
        ...
        m_text33 = wx.StaticText(
            panel, -1, "Number of processors to use", style=wx.ALIGN_LEFT
        )
        ...
        m_text34 = wx.StaticText(
            panel, -1, "Plotting and Diagnostics", style=wx.ALIGN_LEFT
        )
    ...
    def OnEnsembleFileOpen(self, event):
        ...
        dialog = wx.FileDialog(
            None, wildcard="*.dat;*.txt", defaultDir="./", style=wx.OPEN
        )
    ...
    def OnFinidatOpen(self, event):
        ...
        dialog = wx.FileDialog(
            None, wildcard="Restart files|*clm2*r*.nc", defaultDir=rundir, style=wx.OPEN
        )
        result = dialog.ShowModal()
        if result == wx.ID_OK:
    ...
    def OnPftfileOpen(self, event):
        ccsm_input = self.inputdirtxt.GetValue()
        dialog = wx.FileDialog(
            self, defaultDir=ccsm_input + "/lnd/clm2/pftdata", style=wx.OPEN
        )
        result = dialog.ShowModal()
        if result == wx.ID_OK:
    ...
    def OnRun(self, event):
        ...
        if myindex == 0:
            ...
            cmd = (
                ...
                + " --nyears_final_spinup "
                + str(mynyears)
                + " --spinup_vars --diags --model_root /home/"
                + getpass.getuser()
                + "/models/ACME"
            )
            if mycpl_bypass:
...
for filename in os.listdir("./"):
    if (
        Contains(filename, "sitedata.txt")
        and str(filename)[0] != "."
        and str(filename)[0] != "#"
    ):
