...
parser.add_option("--project", dest="project", default="", help="Set project")
parser.add_option(
...
parser.add_option(
    "--pft", dest="mypft", default=-1, help="Use this PFT for all gridcells"
)
parser.add_option(
    "--parm_file", dest="parm_file", default="", help="file for parameter modifications"
)
parser.add_option(
...
parser.add_option("--machine", dest="machine", default="", help="machine to\n")
parser.add_option(
...
parser.add_option(
    "--debugq", dest="debug", default=False, action="store_true", help="Use debug queue"
)
parser.add_option(
    "--srcmods_loc",
    dest="srcmods_loc",
...
parser.add_option(
    ...
    default=False,
    action="store_true",
    help="Livneh correction to CRU precip (CONUS only)",
)
parser.add_option(
    "--daymet",
    dest="daymet",
    default=False,
    action="store_true",
    help="Daymet correction to GSWP3 precip (CONUS only)",
)
parser.add_option(
    "--era5",
    dest="era5",
...
parser.add_option(
    ...
    dest="finidat",
    default="",
    help="initial data file to use" + " (should be in your run directory)",
)
parser.add_option(
    "--finidat_year",
    dest="finidat_year",
    default=-1,
    help="model year of initial data file (default is" + " last available)",
)
parser.add_option(
    "--run_units",
    dest="run_units",
...
parser.add_option(
    ...
    default=False,
    action="store_true",
    help="Remove old case directory with same name" + " before proceeding",
)
...
parser.add_option(
    ...
    default=False,
    action="store_true",
    help="Write daily output for hydrology",
)
parser.add_option(
    "--hist_mfilt",
    dest="hist_mfilt",
...
parser.add_option(
    "--hist_nhtfrq", dest="hist_nhtfrq", default=-999, help="output file timestep"
)
parser.add_option(
    "--hist_vars",
    dest="hist_vars",
...
parser.add_option(
    "--ng", dest="ng", default=64, help="number of groups to run in ensmble mode"
)
parser.add_option("--tstep", dest="tstep", default=0.5, help="model timestep (hours)")
...
parser.add_option(
    ...
    dest="harvmod",
    default=False,
    help="Turn on harvest modificatonAll harvest is performed in first timestep",
)
parser.add_option(
    "--no_dynroot",
    dest="no_dynroot",
...
parser.add_option(
    ...
    dest="ndep1850",
    default=False,
    help="Use constant 1850 N deposition",
    action="store_true",
)
parser.add_option(
    "--ndep_rcp85",
...
parser.add_option(
    ...
    dest="aero1850",
    default=False,
    help="Use constant 1850 aerosol deposition",
    action="store_true",
)
parser.add_option(
    "--aero_rcp85",
    dest="aerorcp85",
    default=False,
    help="Use RCP8.5 aerosol deposition",
    action="store_true",
)
parser.add_option(
    "--1850_co2",
    dest="co21850",
    default=False,
    help="Use constant 1850 CO2 concentration",
    action="store_true",
)
parser.add_option(
    "--C13",
...
parser.add_option(
    ...
    dest="spinup_vars",
    default=False,
    help="Limit output vars in spinup runs",
    action="store_true",
)
parser.add_option(
...
parser.add_option(
    ...
    dest="c_only",
    default=False,
    help="Carbon only (supplemental P and N)",
    action="store_true",
)
parser.add_option(
    "--cn_only",
    dest="cn_only",
    default=False,
    help="Carbon/Nitrogen only (supplemental P)",
    action="store_true",
)
parser.add_option(
    "--cp_only",
    dest="cp_only",
    default=False,
    help="Carbon/Phosphorus only (supplemental N)",
    action="store_true",
)
parser.add_option(
    "--ensemble_file",
...
parser.add_option(
    "--ensemble_nocopy",
    dest="ensemble_nocopy",
    default=False,
    help="Do not copy files to ensemble directories",
    action="store_true",
)
parser.add_option("--surffile", dest="surffile", default="", help="Surface file to use")
parser.add_option(
...
parser.add_option(
    ...
    default=False,
    help="Use variable soil thickness from surface data",
    action="store_true",
)
parser.add_option(
    "--no_budgets",
...
parser.add_option(
    "--dust_snow_mixing",
    dest="dust_snow_mixing",
    default=False,
    help="Use Hao et al. dust/snow mixing albedo parameterization",
    action="store_true",
)
parser.add_option(
    "--no_snicar_ad",
    dest="no_snicar_ad",
    default=False,
    help="Turn off SNICAR-AD snow microphysics model",
    action="store_true",
)
parser.add_option(
    "--use_extra_snow_layers",
    dest="use_extra_snow_layers",
    default=False,
    help="Turn on extra snow layers",
    action="store_true",
)
parser.add_option(
    "--use_firn_percolation_and_compaction ",
    dest="use_firn_percolation_and_compaction",
    default=False,
    help="Turn on firn percolation and compaction",
    action="store_true",
)
# polygonal tundra:
parser.add_option(
    "--use_polygonal_tundra",
    dest="use_polygonal_tundra",
    default=False,
...
parser.add_option(
    ...
    default=False,
    help="Use colder and saturated initial conditions, NGEE Arctic IM2 and IM0",
    action="store_true",
)
# Arctic hillslope hydrology
parser.add_option(
    "--use_IM2_hillslope_hydrology",
    dest="use_IM2_hillslope_hydrology",
    default=False,
    help="Use NGEE Arctic Hillslope Hydrology across topounits",
    action="store_true",
)
# adjust topounit/pft output:
parser.add_option(
    "--arctic_topounit_output",
    dest="arctic_topounit_output",
    default=False,
    action="store_true",
...
parser.add_option(
    ...
    dest="use_onset_gdd_extension",
    default=False,
    help="Extend leaf onset based on accumulated growing degree days past summer solstice in Arctic",
    action="store_true",
)
...
parser.add_option(
    ...
    dest="use_lake_wat_storage",
    default=False,
    help="Turn on use_lake_wat_storage namelist flag",
    action="store_true",
)
...
if options.makepointdata_only:
    ...
    options.domainfile != ""
    and options.surffile != ""
    and (options.nopftdyn or options.pftdynfile != "")
):
...
if "titan" in options.machine:
    ...
    "oic5" in options.machine
    or "cori-haswell" in options.machine
    or "eos" in options.machine
...
if options.mymodel == "":
    if "clm5" in options.csmdir:
        ...
        "E3SM" in options.csmdir or "e3sm" in options.csmdir or "ACME" in options.csmdir
    ):
        options.mymodel = "ELM"
    else:
...
if options.exit_spinup:
    if options.mycaseid != "":
        finidat = (
            options.mycaseid + "_" + options.site + "_I1850" + mybgc + "_ad_spinup"
        )
    else:
...
if options.finidat == "" and options.finidat_case == "":  # not user-defined
    ...
    if compset == "I20TRCLM45" + mybgc or compset == "I20TRCRUCLM45" + mybgc:
        if options.mycaseid != "":
            options.finidat_case = (
                options.mycaseid + "_" + options.site + "_I1850CLM45" + mybgc
            )
        else:
    ...
    if compset == "I20TR" + mybgc:
        if options.mycaseid != "":
            options.finidat_case = (
                options.mycaseid + "_" + options.site + "_I1850" + mybgc
            )
        else:
...
if (
    options.metdir != "none"
):  # obviously user-provided met forcing is not reanalysis type
...
for i in range(1, int(options.ninst) + 1):
    ...
    if options.hist_nhtfrq != -999 and not options.diags:
        if options.ad_spinup:
            ...
            if options.dailyvars:
                output.write(
                    " hist_nhtfrq = " + str(options.hist_nhtfrq) + ",-24,-24\n"
                )
                h1varst = "hist_fincl2 = "
                h2varst = "hist_fincl3 = "
                ...
                output.write(
                    " hist_fincl2 = 'TBOT','QBOT','RAIN','SNOW','QBOT','PBOT','WIND','FPSN','QVEGT',"
                    + "'QVEGE','QSOIL','QRUNOFF','QDRAI','QOVER','H2OSFC','ZWT','SNOWDP','H2OSOI','TSOI','TWS',"
                    + "'FSDS','FLDS'\n"
                )
    ...
    elif ("20TR" in compset or options.istrans) and (
        options.trans_varlist != "" or options.ilambvars
    ):
    ...
    if options.onset_gdd_extension:
    ...
    if (
        ...
        if (
            model_name == "clm2"
        ):  # set for older tags/branches (option was removed, this may not capture all tags
...
if not cpl_bypass:
    ...
    for s in myinput:
        if "streams =" in s:
            ...
            if options.istrans or "20TR" in compset:
                mypresaero = (
                    '"datm.streams.txt.presaero.trans_1850-2000 1850 1850 2000"'
                )
            ...
            if options.cruncep:
                myoutput.write(
                    ' streams = "datm.streams.txt.CLMCRUNCEP.Solar '
                    + str(myalign_year)
                    + " "
                    + str(startyear)
                    + " "
                    + str(endyear)
                    + '  ", '
                    + '"datm.streams.txt.CLMCRUNCEP.Precip '
                    + str(myalign_year)
                    + " "
                    + str(startyear)
                    + " "
                    + str(endyear)
                    + '  ", '
                    + '"datm.streams.txt.CLMCRUNCEP.TPQW '
                    + str(myalign_year)
                    + " "
                    + str(startyear)
                    + " "
                ...
                options.gswp3
            ):  # default in elm seems to set myalign year for gswp3 to 12..?
                ...
                myoutput.write(
                    ' streams = "datm.streams.txt.CLM1PT.'
                    + mylsm
                    + "_USRDAT "
                    + str(myalign_year)
                    + " "
    ...
    if options.cruncep == False:
        ...
        for s in myinput:
            if "CLM1PT_data" in s:
                ...
                s = temp.replace(
                    str(numxpts) + "x" + str(numypts) + "pt" + "_" + options.site,
                    "CLM1PT_data",
                )
                temp = s.replace(
                    "TEMPSTRING",
    ...
    if options.ensemble_file != "":
        ...
        if mysubmit_type == "qsub":
            ...
            if options.machine == "cades":
                output_run.write(
                    "#PBS -l nodes="
                    + str(int(math.ceil(np_total / (ppn * 1.0))))
                    + ":ppn="
                    + str(ppn)
                    + "\n"
                )
        ...
        elif ("titan" in options.machine or "eos" in options.machine) and int(
            options.ninst
        ) == 1:
            cmd = (
                "aprun -n "
