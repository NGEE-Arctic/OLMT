...
def esat(t):
    if t[0] > 0:
        myesat = a[0] + t * (
            a[1] + t * (a[2] + t * (a[3] + t * (a[4] + t * (a[5] + t * a[6]))))
        )
    else:
        myesat = b[0] + t * (
            b[1] + t * (b[2] + t * (b[3] + t * (b[4] + t * (b[5] + t * b[6]))))
        )
    return myesat
...
def bypass_format(
    ...
    if calc_qbot:
        ...
        mye = esat(all_hourly["TBOT"][0, :] - 273.15) * all_hourly["RH"][0, :] / 100.0
        all_hourly["VPD"][0, :] = (
            esat(all_hourly["TBOT"][0, :] - 273.15)
            * (1.0 - all_hourly["RH"][0, :] / 100.0)
            * 100.0
        )
        ...
        all_hourly["QBOT"][0, :] = calc_q(mye, all_hourly["PSRF"][0, :] / 100.0)
    ...
    output_data.variables["DTIME"].units = (
        "days since " + str(startyear) + "-01-01 00:00:00"
    )
    output_data.variables["DTIME"].calendar = "noleap"
    n_years = endyear - startyear + 1
    output_data.variables["DTIME"][:] = (
        np.cumsum(np.ones([int(n_years * 365 * npd)], np.float) / npd) - 0.5 / npd
    )
