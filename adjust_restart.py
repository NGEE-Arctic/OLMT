...
parser.add_option(
    "--rundir", dest="rundir", default="", help="location of run directory"
)
parser.add_option("--casename", dest="casename", default="", help="Name of case")
parser.add_option(
    "--restart_year",
    dest="restart_year",
    default="",
...
if options.restart_year == "":
    ...
    restart_file = glob.glob(
        options.rundir + "/" + casename + "." + options.model_name + ".r.*.nc"
    )
        ...
        for i in range(0, n_rest):
            for j in range(0, 10):
                if (
                    ...
                    and float(hist_vals[0][j][i]) > 1e-10
                ):
