...
for i in range(0, nqoi):
    ...
    if yrange[0, i] != yrange[1, i]:
        ytrain_norm[:, i] = (ytrain[:, i] - yrange[0, i]) / (
...
for n in range(0, 100):
    ...
    for qoi in qoi_good:
        corr_train.append(
            (
                np.corrcoef(
                    ytrain_norm.astype(float)[:, qoi],
                    ypredict_train.astype(float)[:, qoi],
                )[0, 1]
            )
            ** 2
        )
        rmse_train.append(
            (sum((ypredict_train[:, qoi] - ytrain_norm[:, qoi]) ** 2) / ntrain) ** 0.5
        )
        corr_val.append(
            (
                np.corrcoef(
                    yval.astype(float)[:, qoi], ypredict_val.astype(float)[:, qoi]
                )[0, 1]
            )
            ** 2
        )
        rmse_val.append(
            (sum((ypredict_val[:, qoi] - yval_norm[:, qoi]) ** 2) / nval) ** 0.5
        )
    print(corr_val)
    print(rmse_val)
    ...
    if sum(corr_val) > corr_best:
        ...
        for q in qoi_good:
            ...
            plt.scatter(
                yval[:, q],
                ypredict_val_best[:, q] * (yrange[1, q] - yrange[0, q]) + yrange[0, q],
            )
