...
parser.add_option("--ens_num", dest="ensnum", default=1, help="Ensemble member number")
parser.add_option(
    "--parm_list",
    dest="parm_list",
    default="",
...
parser.add_option("--machine", dest="machine", default="cades", help="My machine")
parser.add_option("--casename", dest="casename", default="", help="Name of case to run")
...
for p in parm_names:
    myoutput.write(
        str(parm_names[myind])
        + " "
        + str(parm_indices[myind])
        + " "
        + str(parm_values[myind])
        + "\n"
    )
    myind = myind + 1
            ...
            if lnum == 0:
                ...
                if year <= 2016:
                    ...
                    for f in file_list:
                        if myvarname == "STEMC":
                            os.system(
                                'ncap -s "STEMC=DEADSTEMC+LIVESTEMC" '
                                + f
                                + " "
                                + f
                                + ".tmp"
                            ...
                            if myvarname == "AGBIOMASS":
                                os.system(
                                    'ncap -s "AGBIOMASS=DEADSTEMC+LIVESTEMC+LEAFC" '
                                    + f
                                    + " "
                                    + f
                                    + ".tmp"
