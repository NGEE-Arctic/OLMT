...
parser.add_option(
    "--case_copy", dest="casename", default="", help="Name of case to copy from"
)
parser.add_option(
    "--site_orig", dest="site_orig", default="", help="Site being run in original case"
)
parser.add_option("--site_new", dest="site_new", default="", help="Site to run")
parser.add_option("--nyears", dest="nyears", default=0, help="Number of years to run")
parser.add_option(
    "--finidat_year", dest="finyr", default=0, help="Year for initial data file"
)
parser.add_option(
    "--finidat_thiscase",
    dest="fincase",
    default=False,
    action="store_true",
    help="Use this case for finidat",
)
parser.add_option(
    "--spin_cycle", dest="spin_cycle", default=0, help="Number of years in spinup cycle"
)
parser.add_option(
    "--1850_landuse",
    dest="nolanduse",
    default=False,
    action="store_true",
    help="1850 land use (no dynamics)",
)
parser.add_option(
    "--1850_co2", dest="noco2", default=False, action="store_true", help="use 1850 CO2"
)
parser.add_option(
    "--1850_ndep",
    dest="nondep",
...
parser.add_option("--suffix", dest="suffix", default="", help="use 1850 NDep")
parser.add_option("--machine", dest="machine", default="cades", help="machine")
parser.add_option(
    "--warming", dest="warming", default="0.0", help="warming level to apply"
)
...
for f in os.listdir(new_dir):
    if os.path.isfile(new_dir + "/" + f) and (
        f[-2:] == "in" or f[-3:] == "nml" or "streams" in f
    ):
        ...
        for s in myinput:
            if "drv_in" in f and "stop_n" in s and int(options.nyears) > 0:
                ...
                "lnd_in" in f
                and "finidat =" in s
                and "finidat = ' '" not in s
                and int(options.finyr) > 0
            ):
                print(s)
            ...
            elif "lnd_in" in f and "hist_nhtfrq =" in s and int(options.spin_cycle) > 0:
                ...
                "lnd_in" in f
                and "flanduse_timeseries =" in s
                and options.nolanduse == True
            ):
                s_out = " flanduse_timeseries = ''\n"
            elif (
                "lnd_in" in f and "do_transient_pfts" in s and options.nolanduse == True
            ):
...
if (
    os.path.exists("temp/global_" + options.casename + "_0.pbs")
    and options.suffix != ""
):
    file_in = open("temp/global_" + options.casename + "_0.pbs", "r")
    file_out = open(
        "temp/global_" + options.casename + "_" + options.suffix + ".pbs", "w"
    )
    mpicmd = "mpirun -np " + str(np)
    if "cades" in options.machine:
        mpicmd = (
            "/software/dev_tools/swtree/cs400_centos7.2_pe2016-08/openmpi/1.10.3/centos7.2_gnu5.3.0/bin/mpirun"
            + " -np "
            + str(np)
    ...
    if (
        ...
        or "edison" in options.machine
    ):
        os.system(
            "sbatch temp/global_" + options.casename + "_" + options.suffix + ".pbs"
        )
    else:
        os.system(
            "qsub temp/global_" + options.casename + "_" + options.suffix + ".pbs"
        )
...
if options.site_orig != "":
    ...
    ens_fname = (
        ...
        + case_prefix
        + "/ensemble_run_"
        + options.casename.replace(options.site_orig, options.site_new)
        + ".pbs"
    )

    if os.path.exists(
        "scripts/" + case_prefix + "/ensemble_run_" + options.casename + ".pbs"
    ):
        os.system(
            "cp scripts/"
            + case_prefix
        ...
        for s in myfile:
            ...
            if "--case" in args:
                ...
                print(caseind)
                args[caseind + 1] = args[caseind + 1].replace(
                    options.site_orig, options.site_new
                )
                siteind = args.index("--site")
                args[siteind + 1] = args[siteind + 1].replace(
                    options.site_orig, options.site_new
                )
                sout = " ".join(args)
