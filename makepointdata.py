...
parser.add_option(
    "--model", dest="mymodel", default="", help="Model to use (CLM5 or ELM)"
)
parser.add_option(
    "--mask", dest="mymask", default="", help="Mask file to use (regional only)"
)
parser.add_option(
    "--pft", dest="mypft", default=-1, help="Replace all gridcell PFT with this value"
)
parser.add_option(
    "--point_list",
    dest="point_list",
...
parser.add_option(
    "--res", dest="res", default="hcru_hcru", help="Resolution of global files"
)
parser.add_option(
    "--nopftdyn",
    dest="nopftdyn",
    default=False,
    action="store_true",
    help="Do not make transient PFT file",
)
parser.add_option(
    "--mysimyr", dest="mysimyr", default=1850, help="Simulation year (1850 or 2000)"
)
parser.add_option(
    "--humhol",
    dest="humhol",
...
if "hcru" in options.res:
    ...
    resy = 0.5
    domainfile_orig = (
        ccsm_input + "/share/domains/domain.clm/domain.lnd.360x720_cruncep.100429.nc"
    )
    if options.mymodel == "CLM5":
        ...
        surffile_orig = (
            ccsm_input
            + "/lnd/clm2/surfdata_map/surfdata_360x720cru_24pfts_simyr2000_c150227.nc"
        )
    else:
        if mysimyr == 2000:
            surffile_orig = (
                ccsm_input
                + "/lnd/clm2/surfdata_map/surfdata_360x720cru_simyr2000_c180216.nc"
            )
        else:
            # CMIP6 stype (Hurtt v2)
            surffile_orig = (
                ccsm_input
                + "/lnd/clm2/surfdata_map/surfdata_360x720cru_simyr1850_c180216.nc"
            )
    ...
    surffile_orig = (
        ccsm_input + "/lnd/clm2/surfdata_map/surfdata_1.9x2.5_simyr1850_c180306.nc"
    )
    pftdyn_orig = (
        ccsm_input
        + "/lnd/clm2/surfdata_map/landuse.timeseries_1.9x2.5_rcp8.5_simyr1850-2100_c141219.nc"
    )
    nyears_landuse = 251
    resx = 2.5
    resy = 1.9
    ...
    domainfile_orig = (
        ccsm_input + "/share/domains/domain.lnd.fv0.9x1.25_gx1v6.090309.nc"
    )
    surffile_orig = (
        ccsm_input + "/lnd/clm2/surfdata_map/surfdata_0.9x1.25_simyr1850_c180306.nc"
    )
    pftdyn_orig = (
        ccsm_input
        + "/lnd/clm2/surfdata_map/landuse.timeseries_0.9x1.25_rcp8.5_simyr1850-2100_c141219.nc"
    )
    nyears_landuse = 251
    resx = 1.25
    resy = 0.9
    ...
    domainfile_orig = (
        ccsm_input + "/share/domains/domain.lnd.ne30np4_oEC60to30v3.161222.nc"
    )
    # surffile_orig   = ccsm_input+'/lnd/clm2/surfdata_map/surfdata_ne30np4_simyr1850_2015_c171018.nc'
    surffile_orig = (
        ccsm_input + "/lnd/clm2/surfdata_map/surfdata_ne30np4_simyr1850_c180306.nc"
    )
    pftdyn_orig = (
        ccsm_input
...
if lat_bounds[0] > -90 and lon_bounds[0] > -180:
    ...
    if lon_bounds[0] < 0:
        lon_bounds[0] = lon_bounds[0] + 360.0
    if lon_bounds[1] < 0:
        lon_bounds[1] = lon_bounds[1] + 360.0
    ...
    if options.usersurfnc != "none":
        if options.usersurfvar == "none":
            print(
                "must provide variable name(s) for extracting data from : ",
                options.usersurfnc,
            )
            sys.exit()
        else:
    ...
    for s in input_file:
        ...
        n_grids = n_grids + 1
        if divmod(n_grids, 100)[1] == 0:
            print("grid counting: \n", n_grids)

    # overrides data from a PCT_PFT nc input file, after all index are assembled
    if options.usersurfnc != "none" and options.usersurfvar != "none":
        for isurfvar in mysurfvar:
            isurfvar_vals = numpy.asarray(mysurfnc[isurfvar])[
                :, point_ij[:, 0], point_ij[:, 1]
            ]
    ...
    issite = True
    AFdatareader = csv.reader(
        open(ccsm_input + "/lnd/clm2/PTCLM/" + options.sitegroup + "_sitedata.txt", "r")
    )
    for row in AFdatareader:
        if row[0] == options.site:
            ...
            if "US-SPR" in options.site or (options.humhol):
                lon.append(mylon)
                lat.append(float(row[4]))
                n_grids = 2
            startyear = int(row[6])
...
for n in range(0, n_grids):
    ...
    if "ne" in options.res:
        if lon_bounds[0] != lon_bounds[1] or lat_bounds[0] != lat_bounds[1]:
            print(
                "Regional subsets not allowed for ne resolutions.  Use point lists instead"
            )
            sys.exit()
        ygrid_min[n] = 0
        ygrid_max[n] = 0
...
if n_grids > 1 and options.site == "":  # remove duplicate points
    ...
    myoutput.write(
        ...
        + str(point_index[0])
        + ","
        + str(xgrid_min_uniq[0])
        + ","
        + str(ygrid_min_uniq[0])
        + "\n"
    )
    ...
    for n in range(1, n_grids):
        ...
        if not options.keep_duplicates:
            ...
            if len(xidx[0]) > 0:  # more than 0 indicates duplicated 'x'
                ...
                if len(yidx[0]) > 0:  # both 'x','y' have duplicated points
                    pidx = numpy.where(
                        numpy.asarray(point_pfts_uniq)[xidx[0][yidx]] == point_pfts[n]
                    )
                    if len(pidx[0]) > 0:
                        n_dups = n_dups + 1
                        is_unique = False
        if is_unique:
            ...
            myoutput.write(
                ...
                + str(lat[n])
                + ","
                + str(point_index[n_grids_uniq - 1])
                + ","
                + str(xgrid_min_uniq[n_grids_uniq - 1])
                + ","
                + str(ygrid_min_uniq[n_grids_uniq - 1])
                + "\n"
            )
...
for n in range(0, n_grids):
    ...
    if options.point_area_km2 is not None:
    ...
    if options.point_area_deg2 is not None:
        area_n = area_orig[ygrid_min[n], xgrid_min[n]]
        if float(area_n) > 0.0:
            side_deg = math.sqrt(
                float(options.point_area_deg2)
            )  # degx X degy, NOT square radians
    ...
    if issite:
        ...
        if options.site != "":
        ...
        elif options.point_area_km2 is not None or options.point_area_deg2 is not None:
            xc[0] = lon[n]
            yc[0] = lat[n]
            ...
            if options.point_area_km2 is not None:
                area[0] = (
                    float(options.point_area_km2) / re_km / re_km
                )  # there is about 0.3% error with calculation above
            ierr = nffun.putvar(domainfile_new, "xc", xc)
            ierr = nffun.putvar(domainfile_new, "yc", yc)
            ierr = nffun.putvar(domainfile_new, "xv", xv)
            ierr = nffun.putvar(domainfile_new, "yv", yv)
...
if n_grids > 1:
    ...
    ierr = os.system(
        ...
        + ' -name "'
        + domainfile_tmp
        + '" | xargs ls | sort | ncrcat -O -h -o'
        + domainfile_new
    )
    if ierr != 0:
        raise RuntimeError("Error: ncrcat -", ierr)  # os.sys.exit()
    ierr = os.system(
        "nccopy -6 -u " + domainfile_new + " " + domainfile_new + ".tmp"
    )  # NC-3  with large dataset support due to 64bit offset
    if ierr != 0:
        raise RuntimeError("Error: nccopy -6 -u ", ierr)  # os.sys.exit()
    ierr = os.system(
        "ncpdq -h -O -a ni,nj " + domainfile_new + ".tmp " + domainfile_new
    )
    if ierr != 0:
        raise RuntimeError("Error: ncpdq", ierr)  # os.sys.exit()
    ierr = os.system(
        "ncrename -h -O -d ni,ni_temp " + domainfile_new + " " + domainfile_new + " "
    )
    if ierr != 0:
        raise RuntimeError("Error: ncrename", ierr)  # os.sys.exit()
    ierr = os.system(
        "ncrename -h -O -d nj,ni " + domainfile_new + " " + domainfile_new + " "
    )
    if ierr != 0:
        raise RuntimeError("Error: ncrename", ierr)  # os.sys.exit()
    ierr = os.system(
        "ncrename -h -O -d ni_temp,nj " + domainfile_new + " " + domainfile_new + " "
    )
    if ierr != 0:
        raise RuntimeError("Error: ncrename", ierr)  # os.sys.exit()
...
for n in range(0, n_grids):
    ...
    if isglobal:
        ...
        if "ne" in options.res:
            os.system(
                "ncks -h -O --fix_rec_dmn time -d gridcell,"
                + str(xgrid_min[n])
                + ","
                + str(xgrid_max[n])
                + " "
            ...
            os.system(
                "ncks -h -O --fix_rec_dmn time -d lsmlon,"
                + str(xgrid_min[n])
                + ","
                + str(xgrid_max[n])
                + " -d lsmlat,"
    ...
    if issite:
        ...
        if options.mymodel == "CLM5" or options.crop:
            ...
            for v in vars_in:
                tempvar = tempdata.createVariable(
                    v,
                    "f4",
                    (
                        "lsmlat",
        ...
        if not options.surfdata_grid and options.site != "":
            # read file for site-specific PFT information
            AFdatareader = csv.reader(
                open(
                    ccsm_input
            ...
            try:
                ...
                if "PCT_PFT" in mysurfvar or "PCT_NAT_PFT" in mysurfvar:
                    ...
                    if "PCT_PFT" in point_mysurf.keys():
                        ...
                        print(
                            "Error: PCT_PFT or PCT_NAT_PFT is used variable name for PFT fraction in surface data"
                        )
                        sys.exit()
        ...
        if options.site != "":
            ...
            area[0] = (
                ...
                * resy
                * 111.321
                * math.cos((lat[n] * resx) * math.pi / 180)
                * resx
            )
        elif options.point_area_km2 is not None:
            longxy[0][0] = lon[n]
            latixy[0][0] = lat[n]
            area[0] = float(options.point_area_km2)
        elif (
            options.point_area_deg2 is not None
        ):  # degx X degy (NOT square radians of area)
            longxy[0][0] = lon[n]
            latixy[0][0] = lat[n]
            side_deg = math.sqrt(
                float(options.point_area_deg2)
            )  # a square of lat/lon degrees assummed
            area[0][0] = (
                111.2
                * side_deg
                * 111.321
                * math.cos((lat[n] * side_deg) * math.pi / 180)
                * side_deg
            )
...
if n_grids > 1:
    ...
    # remove ni dimension
    ierr = os.system(
        "ncwa -h -O -a lsmlat -d lsmlat,0,0 "
        + surffile_new
        + " "
        + surffile_new
        + ".tmp"
    ...
    if ierr != 0:
        raise RuntimeError("Error: ncwa ")  # os.sys.exit()
    ierr = os.system(
        "nccopy -6 -u " + surffile_new + ".tmp" + " " + surffile_new + ".tmp2"
    )  # NC-3 with large dataset support (64bit offset)
    if ierr != 0:
        raise RuntimeError("Error: nccopy -6 -u ")  # os.sys.exit()
    ierr = os.system(
        "ncpdq -h -a lsmlon,record " + surffile_new + ".tmp2 " + surffile_new + ".tmp3"
    )
    if ierr != 0:
        raise RuntimeError("Error: ncpdq ")  # os.sys.exit()
    ierr = os.system(
        "ncwa -h -O -a lsmlon -d lsmlon,0,0 "
        + surffile_new
        + ".tmp3 "
        + surffile_new
        + ".tmp4"
    ...
    if ierr != 0:
        raise RuntimeError("Error: ncwa ")  # os.sys.exit()
    ierr = os.system(
        "ncrename -h -O -d record,gridcell "
        + surffile_new
        + ".tmp4 "
        + surffile_new
        + ".tmp5"
    ...
    if ierr != 0:
        raise RuntimeError("Error: ncrename ")  # os.sys.exit()
...
if not options.nopftdyn:
    ...
    for n in range(0, n_grids):
        ...
        if issite:
            ...
            if not options.surfdata_grid and options.site != "":
                ...
                if os.path.exists(
                    ccsm_input + "/lnd/clm2/PTCLM/" + options.site + "_dynpftdata.txt"
                ):
                    dynexist = True
                    DYdatareader = csv.reader(
                        open(
                            ccsm_input
                            + "/lnd/clm2/PTCLM/"
                            + options.site
                    ...
                    print(
                        "Warning:  Dynamic pft file for site "
                        + options.site
                        + " does not exist"
                    )
            ...
            for t in range(0, nyears_landuse):
                if not options.surfdata_grid:
                    if dynexist:
                        ...
                        for k in range(0, 5):
                            ...
                            if harvest_thisyear:
                                ...
                                harvest_sh1[t][0][0] = 0.0
                                harvest_sh2[t][0][0] = 0.0
                                harvest_sh3[t][0][0] = 0.0
                    ...
                    if options.usersurfnc != "none" and (
                        "PCT_PFT" in mysurfvar or "PCT_NAT_PFT" in mysurfvar
                    ):
                        print(
                            "Message: PCT_NAT_PFT is extracted from a non-dynamical surface data FOR surfdata.pftdyn.nc"
                        )
                        sum_nat = numpy.sum(
                            pct_pft[t, :, 0, 0]
                        ...
                        if "PCT_PFT" in point_mysurf.keys():
                            ...
                            print(
                                "Error: PCT_PFT or PCT_NAT_PFT is used variable name for PFT fraction in non-dynamical surface data"
                            )
                            sys.exit()
                        # in case new PCT not summed to 100.0
                        sum_nat2 = numpy.sum(
                            pct_pft[t, :, 0, 0]
                        )  # this is the updated above
                        if sum_nat2 != 100.0:
                            adj = 100.0 / sum_nat2
    ...
    if n_grids > 1:
        ...
        if ierr != 0:
            raise RuntimeError("Error: ncwa ")  # os.sys.exit()
        ierr = os.system(
            "nccopy -6 -u " + pftdyn_new + ".tmp" + " " + pftdyn_new + ".tmp2"
        )  # NC-3 with large dataset support due to 64bit offset
        if ierr != 0:
            raise RuntimeError("Error: nccopy -6 -u ")  # os.sys.exit()
        ierr = os.system(
            "ncpdq -h -a lsmlon,record " + pftdyn_new + ".tmp2 " + pftdyn_new + ".tmp3"
        )
        if ierr != 0:
            raise RuntimeError("Error: ncpdq ")  # os.sys.exit()
        ierr = os.system(
